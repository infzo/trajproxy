#!/bin/bash
# 场景 002: 基础chat用例
# 测试流程：12345端口注册带run-id模型 -> 发送非流式推理请求 -> 删除模型
...
# 步骤 2: 发送非流式推理请求
log_step "步骤 2: 发送非流式推理请求（session_id: ${CHAT_TEST_SESSION_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
...
CHAT_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${CHAT_TEST_BASE_URL}/s/${CHAT_TEST_SESSION_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${CHAT_TEST_MODEL_NAME}\",
