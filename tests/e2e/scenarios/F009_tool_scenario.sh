#!/bin/bash
# 场景 009: Tool场景
# 测试流程：12345端口注册模型（带tool_parser） -> 发送带tools的非流式推理请求 -> 验证响应中tool_calls字段非空 -> 删除模型
...
TOOL_TEST_SESSION_ID="${TOOL_TEST_RUN_ID},sample009,task009"
...
log_step "步骤 2: 发送带 tools 的非流式推理请求（session_id: ${TOOL_TEST_SESSION_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${TOOL_TEST_BASE_URL}/s/${TOOL_TEST_SESSION_ID}/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
...
CHAT_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${TOOL_TEST_BASE_URL}/s/${TOOL_TEST_SESSION_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${TOOL_TEST_MODEL_NAME}\",
...
# 步骤 3: 查询轨迹确认存在
log_step "步骤 3: 查询轨迹（session_id: ${TOOL_TEST_SESSION_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
