#!/bin/bash
# 场景 005: 流式场景
# 测试流程：12346端口注册带run-id模型 -> 发送流式推理请求 -> 等待返回完成 -> 删除模型
...
# 测试配置
CHAT_TEST_PORT=12345
CHAT_TEST_BASE_URL="http://127.0.0.1:${CHAT_TEST_PORT}"
CHAT_TEST_MODEL_NAME="stream-test-model"
CHAT_TEST_RUN_ID="stream-run-005"
CHAT_TEST_SESSION_ID="${CHAT_TEST_RUN_ID},sample001,task001"
...
# 步骤 2: 发送流式推理请求
log_step "步骤 2: 发送流式推理请求（session_id: ${CHAT_TEST_SESSION_ID}）"
log_curl_cmd "curl -s --no-buffer \\
    -X POST '${CHAT_TEST_BASE_URL}/s/${CHAT_TEST_SESSION_ID}/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
...
# 发送流式请求并收集完整响应
STREAM_RESPONSE=$(curl -s --no-buffer -X POST "${CHAT_TEST_BASE_URL}/s/${CHAT_TEST_SESSION_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${CHAT_TEST_MODEL_NAME}\",
