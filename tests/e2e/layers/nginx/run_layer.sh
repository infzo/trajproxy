#!/bin/bash
# Layer 1 测试运行器: Nginx 入口 (port 12345)
# 用法:
#   ./run_layer.sh           # 运行本层所有测试
#   ./run_layer.sh F100      # 运行指定场景
#   ./run_layer.sh F100 F101 # 运行多个场景

# 获取脚本目录
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCENARIOS_DIR="${SCRIPT_DIR}/scenarios"

LAYER_NAME="Layer 1 - Nginx Entry (port 12345)"
...
print_usage() {
    echo "用法:"
    echo "  $0             运行本层所有测试场景"
    echo "  $0 <场景编号>  运行指定场景（如: $0 F100）"
    echo "  $0 F100 F101   运行多个场景"
    echo ""
    echo "当前层: ${LAYER_NAME}"
    echo ""
    echo "可用的测试场景:"
