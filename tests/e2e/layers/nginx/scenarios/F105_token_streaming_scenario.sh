...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${TOKEN_STREAM_TEST_BASE_URL}/models/register' \\
    -H 'Content-Type: application/json' \\
    -d '{
        \"run_id\": \"${TOKEN_STREAM_TEST_RUN_ID}\",
        \"model_name\": \"${TOKEN_STREAM_TEST_MODEL_NAME}\",
        \"url\": \"${BACKEND_MODEL_URL}\",
        \"api_key\": \"${TEST_MODEL_API_KEY}\",
        \"tokenizer_path\": \"${TOKEN_STREAM_TEST_TOKENIZER_PATH}\",
        \"token_in_token_out\": true
    }'"
log_separator

REGISTER_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${TOKEN_STREAM_TEST_BASE_URL}/models/register" \
    -H "Content-Type: application/json" \
    -d "{
        \"run_id\": \"${TOKEN_STREAM_TEST_RUN_ID}\",
        \"model_name\": \"${TOKEN_STREAM_TEST_MODEL_NAME}\",
...
assert_eq "$TOKEN_STREAM_TEST_RUN_ID" "$REGISTER_RUN_ID" "run_id 应为 ${TOKEN_STREAM_TEST_RUN_ID}"
...
log_curl_cmd "curl -s --no-buffer \\
    ...
    -d '{
        \"model\": \"${TOKEN_STREAM_TEST_MODEL_NAME}\",
...
log_step "步骤 3: 查询轨迹（session_id: ${TOKEN_STREAM_TEST_SESSION_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X GET '${TOKEN_STREAM_TEST_BASE_URL}/trajectory?session_id=${TOKEN_STREAM_TEST_SESSION_ID}&limit=100'"
log_separator

TRAJ_RESPONSE=$(curl -s -w "\n%{http_code}" -X GET "${TOKEN_STREAM_TEST_BASE_URL}/trajectory?session_id=${TOKEN_STREAM_TEST_SESSION_ID}&limit=100")
...
assert_eq "$TOKEN_STREAM_TEST_SESSION_ID" "$TRAJ_SESSION_ID" "session_id 应为 ${TOKEN_STREAM_TEST_SESSION_ID}"
...
#     # 检查是否为空数组
#     if echo "$TRAJ_BODY" | grep -q '"response_ids"[[:space:]]*:[[:space:]]*\[\]'; then
#         log_error "response_ids 为空数组，Token流式模式应有响应token"
#         TEST_FAILED=1
#     else
#         log_success "response_ids 不为空，Token流式模式工作正常"
#     fi
# else
...
# 步骤 4: 删除模型
log_step "步骤 4: 删除模型（run_id: ${TOKEN_STREAM_TEST_RUN_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X DELETE '${TOKEN_STREAM_TEST_BASE_URL}/models?model_name=${TOKEN_STREAM_TEST_MODEL_NAME}&run_id=${TOKEN_STREAM_TEST_RUN_ID}'"
log_separator

DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE "${TOKEN_STREAM_TEST_BASE_URL}/models?model_name=${TOKEN_STREAM_TEST_MODEL_NAME}&run_id=${TOKEN_STREAM_TEST_RUN_ID}")
