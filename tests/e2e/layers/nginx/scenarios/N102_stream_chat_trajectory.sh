#!/bin/bash
# 场景 F204: 流式 chat 轨迹存储（降级冒烟，Nginx 层）
# 来源: 原F102降级为轻量冒烟; 测试流程：注册模型 -> 流式请求 -> 验证响应 -> 删除模型
...
echo "========================================"
echo "场景 F204: 流式 chat 轨迹存储（降级冒烟，Nginx 层）"
echo "========================================"
echo ""
...
# 发送流式请求并收集完整响应
STREAM_RESPONSE=$(curl_with_log -s --no-buffer -X POST "${CHAT_TEST_BASE_URL}/s/${CHAT_TEST_RUN_ID}/${CHAT_TEST_SESSION_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" \
