#!/bin/bash
# 场景 C300: 直接转发模式 - 纯文本 API 一致性验证
# 测试流程：注册模型(Direct) -> 非流式对比 -> 流式对比 -> 删除模型
# 核心看护: 透传模式下 trajproxy 响应与 vLLM 原始响应的每个字段一致性

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/../utils.sh"

echo "========================================"
echo "场景 C300: 直接转发模式 - 纯文本 API 一致性验证"
echo "========================================"
echo ""
...
register_model "$TEST_RUN_ID" "$COMPARISON_MODEL_NAME" "false"
...
# 构建请求体（两者使用相同的 vLLM 模型名，proxy 通过 session 路由使用注册配置）
...
# 对比
compare_nonstream "$VLLM_NS_FILE" "$PROXY_NS_FILE" "C300-direct-plain"
...
# 对比
compare_stream "$VLLM_S_FILE" "$PROXY_S_FILE" "C300-direct-plain-stream"
