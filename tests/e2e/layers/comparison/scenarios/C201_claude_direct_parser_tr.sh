#!/bin/bash
# C201: Claude Direct T+R Parser 一致性 (对等C101)
# Pipeline: Direct | Parser: T+R | 轮次: 单轮 | 格式: Claude (ns+s)
# 关键: Direct 模式下 thinking + tool_use 全字段对比

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/../utils.sh"

echo "=== C201: Claude Direct T+R Parser 一致性 ==="
...
compare_claude_nonstream "$VLLM_NS" "$PROXY_NS" "C201-claude-direct-reasoning-tool"
...
compare_claude_stream "$VLLM_S" "$PROXY_S" "C201-claude-direct-reasoning-tool-stream"
rm -f "$VLLM_S" "$PROXY_S"
