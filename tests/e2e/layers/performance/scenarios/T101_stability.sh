...
echo "========================================"
echo "场景 T101: 长稳测试（Performance 层）"
echo "========================================"
echo ""
...
# 先尝试清理残留模型（忽略错误）
curl_with_log -s -X DELETE "${STABILITY_TEST_BASE_URL}/models?model_name=${STABILITY_TEST_MODEL_NAME}&run_id=${STABILITY_TEST_RUN_ID}" > /dev/null 2>&1 || true
...
for ((i=1; i<=STABILITY_TEST_REQUEST_COUNT; i++)); do
    ...
    # 循环中的 curl 保持原始 curl（T101 已有内置计时机制）
