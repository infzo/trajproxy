...
echo "========================================"
echo "场景 F201: PANGU集成场景（Proxy 层）"
echo "========================================"
echo ""
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${PANGU_TEST_BASE_URL}/models/register' \\
...
REGISTER_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${PANGU_TEST_BASE_URL}/models/register" \
    -H "Content-Type: application/json" \
    -d "{
        \"run_id\": \"${PANGU_TEST_RUN_ID}\",
        \"model_name\": \"${PANGU_TEST_MODEL_NAME}\",
...
# 步骤 2: 发送非流式推理请求（model参数格式: {model_name},{run_id}）
log_step "步骤 2: 发送非流式推理请求（model: ${PANGU_TEST_MODEL_PARAM}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${PANGU_TEST_BASE_URL}/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
...
CHAT_RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "${PANGU_TEST_BASE_URL}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${PANGU_TEST_MODEL_PARAM}\",
...
# 步骤 3: 发送流式推理请求（model参数格式: {model_name},{run_id}）
log_step "步骤 3: 发送流式推理请求（model: ${PANGU_TEST_MODEL_PARAM}）"
...
# 发送流式请求并收集完整响应
STREAM_RESPONSE=$(curl -s --no-buffer -X POST "${PANGU_TEST_BASE_URL}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${PANGU_TEST_MODEL_PARAM}\",
...
# 步骤 4: 删除模型
log_step "步骤 4: 删除模型（run_id: ${PANGU_TEST_RUN_ID}）"
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X DELETE '${PANGU_TEST_BASE_URL}/models?model_name=${PANGU_TEST_MODEL_NAME}&run_id=${PANGU_TEST_RUN_ID}'"
log_separator

DELETE_RESPONSE=$(curl -s -w "\n%{http_code}" -X DELETE "${PANGU_TEST_BASE_URL}/models?model_name=${PANGU_TEST_MODEL_NAME}&run_id=${PANGU_TEST_RUN_ID}")
