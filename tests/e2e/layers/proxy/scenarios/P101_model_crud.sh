...
echo "========================================"
echo "场景 F101: 模型注册-列表-删除（Proxy 层）"
echo "========================================"
echo ""
...
REGISTER_A_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X POST "${API_MODELS}/register" \
...
REGISTER_B_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X POST "${API_MODELS}/register" \
    -H "Content-Type: application/json" \
    -d "{
...
LIST_FINAL_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X GET "${API_MODELS}")
