...
STREAM_RESPONSE=$(curl_with_log -s --no-buffer -X POST "${CONSISTENCY_TEST_BASE_URL}/s/${CONSISTENCY_TEST_RUN_ID}/${CONSISTENCY_TEST_SESSION_ID_STREAM}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${CONSISTENCY_TEST_MODEL_NAME}\",
