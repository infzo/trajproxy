...
%{http_code}" -X POST "${BASE_URL}/s/${RUN_ID}/${SESS_ID}/v1/chat/completions" \
    ...
    -d "{\"model\":\"${MODEL_NAME}\",\"messages\":[{\"role\":\"user\",\"content\":\"Weather in ${CITIES[0]}? Think first then call tool.\"}],\"tools\":${TOOLS},\"max_tokens\":512,\"chat_template_kwargs\":{\"preserve_thinking\":true,\"enable_thinking\":true}}")
...
%{http_code}" -X POST "${BASE_URL}/s/${RUN_ID}/${SESS_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{\"model\":\"${MODEL_NAME}\",\"messages\":${R3_MESSAGES},\"tools\":${TOOLS},\"max_tokens\":512,\"chat_template_kwargs\":{\"preserve_thinking\":true,\"enable_thinking\":true}}")
