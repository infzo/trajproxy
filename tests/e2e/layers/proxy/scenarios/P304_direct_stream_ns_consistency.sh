...
# 步骤 1: 注册模型（直接转发模式）
# ========================================
log_step "步骤 1: 注册模型（直接转发模式, token_in_token_out: false）"

REGISTER_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X POST "${DIRECT_TEST_BASE_URL}/models/register" \
    -H "Content-Type: application/json" \
    -d "{
        \"run_id\": \"${DIRECT_TEST_RUN_ID}\",
        \"model_name\": \"${DIRECT_TEST_MODEL_NAME}\",
...
# ========================================
# 步骤 3: 发送流式请求（相同 prompt，直接转发模式）
# ========================================
log_step "步骤 3: 发送流式请求（session_id: ${DIRECT_TEST_SESSION_ID_STREAM}）"

STREAM_RESPONSE=$(curl_with_log -s --no-buffer -X POST "${DIRECT_TEST_BASE_URL}/s/${DIRECT_TEST_RUN_ID}/${DIRECT_TEST_SESSION_ID_STREAM}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${DIRECT_TEST_MODEL_NAME}\",
...
# 步骤 4: 查询非流式轨迹
# ========================================
log_step "步骤 4: 查询非流式轨迹（session_id: ${DIRECT_TEST_SESSION_ID_NONSTREAM}）"

NONSTREAM_TRAJ_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X GET "${DIRECT_TEST_BASE_URL}/trajectory?session_id=${DIRECT_TEST_SESSION_ID_NONSTREAM}&limit=100")
...
if [ "$NONSTREAM_TRAJ_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    assert_fail "非流式轨迹记录数应为大于0" "实际: ${NONSTREAM_TRAJ_COUNT}"
fi
...
# 步骤 5: 查询流式轨迹
# ========================================
log_step "步骤 5: 查询流式轨迹（session_id: ${DIRECT_TEST_SESSION_ID_STREAM}）"

STREAM_TRAJ_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X GET "${DIRECT_TEST_BASE_URL}/trajectory?session_id=${DIRECT_TEST_SESSION_ID_STREAM}&limit=100")
...
if [ "$STREAM_TRAJ_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    assert_fail "流式轨迹记录数应为大于0" "实际: ${STREAM_TRAJ_COUNT}"
fi
...
# 步骤 6: 比较流式与非流式轨迹一致性
# ========================================
log_step "步骤 6: 比较流式与非流式轨迹字段一致性（直接转发模式）"
log_separator

# 使用 python3 进行 JSON 级别的字段比较
# 通过临时文件传递 JSON，避免命令行参数长度超限
DIRECT_NS_TMPFILE=$(mktemp)
DIRECT_S_TMPFILE=$(mktemp)
echo "$NONSTREAM_TRAJ_BODY" > "$DIRECT_NS_TMPFILE"
echo "$STREAM_TRAJ_BODY" > "$DIRECT_S_TMPFILE"

DIRECT_CONSISTENCY_RESULT=$(python3 -c '
import json
import sys
...
# 直接转发模式下可能为空的 token 相关字段
DIRECT_MODE_OPTIONAL_FIELDS = {
    "token_ids", "prompt_text", "response_ids",
...
def compare_recursive(ns_val, s_val, path, errors, info, *,
    ...
    # 直接转发模式可选字段：允许一方为空
    if allow_optional_empty and (ns_emp or s_emp):
        if ns_emp and s_emp:
            info.append(f"  {path}: 两者均为空 (直接转发模式可选字段)")
        else:
            which_empty = "非流式" if ns_emp else "流式"
            info.append(f"  {path}: {which_empty}为空 (直接转发模式允许)")
        return
...
def check_field_consistency(nonstream_body, stream_body):
    ...
    # ---- 响应文本字段：仅检查非空 ----
    response_fields = ["response_text"]
    for field in response_fields:
        compare_recursive(ns_rec.get(field), s_rec.get(field), field,
                          errors, info, value_eq=False)

    # ---- 直接转发模式可选的 token 相关字段：允许一方为空 ----
    for field in DIRECT_MODE_OPTIONAL_FIELDS:
        ns_val = ns_rec.get(field)
        s_val = s_rec.get(field)
        compare_recursive(ns_val, s_val, field, errors, info,
                          value_eq=False, allow_optional_empty=True)
    ...
    for field in token_stat_fields:
        ...
        if ns_has and s_has:
            ...
            info.append(f"  {field}: 两者均缺失 (直接转发模式允许)")
        else:
            info.append(f"  {field}: 仅一方存在 (直接转发模式允许)")
...
# 步骤 7: 删除模型
# ========================================
log_step "步骤 7: 删除模型（run_id: ${DIRECT_TEST_RUN_ID}）"

DELETE_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" -X DELETE "${DIRECT_TEST_BASE_URL}/models?model_name=${DIRECT_TEST_MODEL_NAME}&run_id=${DIRECT_TEST_RUN_ID}")
