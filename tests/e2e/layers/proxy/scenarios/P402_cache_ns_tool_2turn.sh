...
%{http_code}" -X POST "${BASE_URL}/s/${RUN_ID}/${SESS_ID}/v1/chat/completions" \
    ...
    -d "{\"model\":\"${MODEL_NAME}\",\"messages\":[{\"role\":\"user\",\"content\":\"What is the weather in Shanghai?\"}],\"tools\":${TOOLS},\"max_tokens\":512,\"chat_template_kwargs\":{\"preserve_thinking\":true,\"enable_thinking\":true}}")
