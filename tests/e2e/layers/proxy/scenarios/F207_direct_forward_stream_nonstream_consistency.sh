...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${DIRECT_TEST_BASE_URL}/models/register' \\
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X GET '${DIRECT_TEST_BASE_URL}/trajectory?session_id=${DIRECT_TEST_SESSION_ID_NONSTREAM}&limit=100'"
log_separator
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X GET '${DIRECT_TEST_BASE_URL}/trajectory?session_id=${DIRECT_TEST_SESSION_ID_STREAM}&limit=100'"
log_separator
...
if [ -z "$DIRECT_ERRORS" ]; then
    ...
    log_error "直接转发模式下流式与非流式轨迹存在不一致:"
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X DELETE '${DIRECT_TEST_BASE_URL}/models?model_name=${DIRECT_TEST_MODEL_NAME}&run_id=${DIRECT_TEST_RUN_ID}'"
log_separator
