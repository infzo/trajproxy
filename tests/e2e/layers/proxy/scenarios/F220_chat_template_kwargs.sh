#!/bin/bash
# 场景 F220: chat_template_kwargs 透传与消费测试（Proxy 层）
...
echo "========================================"
echo "场景 F220: chat_template_kwargs 透传与消费测试（Proxy 层）"
echo "========================================"
echo ""
...
# ========================================
log_info "清理可能残留的模型..."
curl -s --noproxy '*' -X DELETE "${TEST_BASE_URL}/models?model_name=${DIRECT_MODEL_NAME}&run_id=${DIRECT_RUN_ID}" > /dev/null 2>&1
curl -s --noproxy '*' -X DELETE "${TEST_BASE_URL}/models?model_name=${TITO_MODEL_NAME}&run_id=${TITO_RUN_ID}" > /dev/null 2>&1
...
log_response "HTTP Status: ${DIRECT_REG_STATUS}"
log_response "${DIRECT_REG_BODY}"
log_separator
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${TEST_BASE_URL}/s/${DIRECT_RUN_ID}/${DIRECT_SESSION_ID}/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
...
log_response "HTTP Status: ${DIRECT_STATUS}"
log_response "${DIRECT_BODY}"
log_separator
...
log_curl_cmd "curl -s --no-buffer \\
    -X POST '${TEST_BASE_URL}/s/${DIRECT_RUN_ID}/${DIRECT_SESSION_ID}-stream/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
    -H 'Authorization: Bearer ${CHAT_API_KEY}' \\
    -d '{
        \"model\": \"${DIRECT_MODEL_NAME}\",
...
DIRECT_STREAM_RESP=$(curl -s --noproxy '*' --no-buffer --max-time 15 -X POST "${TEST_BASE_URL}/s/${DIRECT_RUN_ID}/${DIRECT_SESSION_ID}-stream/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${DIRECT_MODEL_NAME}\",
...
echo "$DIRECT_STREAM_RESP" | head -5
log_separator
...
TITO_REG_RESP=$(curl -s --noproxy '*' --max-time 10 -w "\n%{http_code}" -X POST "${TEST_BASE_URL}/models/register" \
    -H "Content-Type: application/json" \
    -d "{
        \"run_id\": \"${TITO_RUN_ID}\",
        \"model_name\": \"${TITO_MODEL_NAME}\",
...
log_response "HTTP Status: ${TITO_REG_STATUS}"
log_response "${TITO_REG_BODY}"
log_separator
...
log_curl_cmd "curl -s -w '\n%{http_code}' \\
    -X POST '${TEST_BASE_URL}/s/${TITO_RUN_ID}/${TITO_SESSION_ID}/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
    -H 'Authorization: Bearer ${CHAT_API_KEY}' \\
    -d '{
...
log_response "HTTP Status: ${TITO_STATUS}"
log_response "${TITO_BODY}"
log_separator
...
log_curl_cmd "curl -s --no-buffer \\
    -X POST '${TEST_BASE_URL}/s/${TITO_RUN_ID}/${TITO_SESSION_ID}-stream/v1/chat/completions' \\
    -H 'Content-Type: application/json' \\
    -H 'Authorization: Bearer ${CHAT_API_KEY}' \\
    -d '{
        \"model\": \"${TITO_MODEL_NAME}\",
...
TITO_STREAM_RESP=$(curl -s --noproxy '*' --no-buffer --max-time 15 -X POST "${TEST_BASE_URL}/s/${TITO_RUN_ID}/${TITO_SESSION_ID}-stream/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${TITO_MODEL_NAME}\",
