...
FIELDS_INCLUDE_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" \
...
if [ "$RECORD_MODEL_COUNT" -gt 0 ] 2>/dev/null; then
    ...
    assert_fail "fields include: 'model' 字段应存在于记录中"
fi
...
if echo "$FIELDS_INCLUDE_BODY" | grep -q '"messages"'; then
    assert_fail "fields include: 不应包含未请求的 'messages' 字段"
else
...
if echo "$FIELDS_INCLUDE_BODY" | grep -q '"raw_request"'; then
    assert_fail "fields include: 不应包含未请求的 'raw_request' 字段"
else
...
FIELDS_EXCLUDE_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" \
...
if echo "$FIELDS_EXCLUDE_BODY" | grep -q '"raw_request"'; then
    assert_fail "fields exclude: 不应包含 'raw_request' 字段"
else
...
if echo "$FIELDS_EXCLUDE_BODY" | grep -q '"raw_response"'; then
    assert_fail "fields exclude: 不应包含 'raw_response' 字段"
else
...
if echo "$FIELDS_EXCLUDE_BODY" | grep -q '"messages"'; then
    assert_fail "fields exclude: 不应包含 'messages' 字段"
else
...
FIELDS_ALL_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" \
...
LEGACY_FIELDS_RESPONSE=$(curl_with_log -s -w "\n%{http_code}" \
...
if echo "$LEGACY_FIELDS_BODY" | grep -q '"messages"'; then
    assert_fail "旧接口 fields: 不应包含未请求的 'messages' 字段"
else
