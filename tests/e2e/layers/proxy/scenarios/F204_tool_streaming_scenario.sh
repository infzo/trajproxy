#!/bin/bash
# 场景 F204: Tool流式场景（Proxy 层）
# 测试流程：注册模型（带tool_parser） -> 发送带tools的流式推理请求 -> 验证流式响应中tool_calls解析 -> 删除模型
...
# 发送流式请求并收集完整响应
STREAM_RESPONSE=$(curl -s --no-buffer -X POST "${TOOL_STREAM_TEST_BASE_URL}/s/${TOOL_STREAM_TEST_RUN_ID}/${TOOL_STREAM_TEST_SESSION_ID}/v1/chat/completions" \
    -H "Content-Type: application/json" \
    -H "Authorization: Bearer ${CHAT_API_KEY}" \
    -d "{
        \"model\": \"${TOOL_STREAM_TEST_MODEL_NAME}\",
