...
#   ./run_layer.sh A100      # 运行指定场景
#   ./run_layer.sh A100 A101 # 运行多个场景

# 获取脚本目录
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCENARIOS_DIR="${SCRIPT_DIR}/scenarios"

LAYER_NAME="Layer Archive - Database Archive Scheduler"
...
print_usage() {
    echo "用法:"
    echo "  $0             运行本层所有测试场景"
    echo "  $0 <场景编号>  运行指定场景（如: $0 A101）"
    echo "  $0 A100 A101   运行多个场景"
    echo ""
    echo "当前层: ${LAYER_NAME}"
    echo ""
    echo "可用的测试场景:"
