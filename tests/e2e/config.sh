...
# 默认推理模型配置（可通过环境变量覆盖）
DEFAULT_MODEL_NAME="${DEFAULT_MODEL_NAME:-Qwen3.6-27B}"
DEFAULT_TOKENIZER_PATH="${DEFAULT_TOKENIZER_PATH:-Qwen/Qwen3.6-27B}"
DEFAULT_TOOL_PARSER="${DEFAULT_TOOL_PARSER:-qwen3_coder}"
...
# 后端推理服务地址（通过环境变量配置）
BACKEND_MODEL_URL="${BACKEND_MODEL_URL:-http://host.docker.internal:8080/v1}"
