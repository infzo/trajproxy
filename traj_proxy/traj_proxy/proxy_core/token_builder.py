...
class TokenBuilder:
    ...
    def __init__(self, model: str, tokenizer_path: str, db_manager=None):
        """初始化 TokenBuilder

        Args:
            model: 模型名称
            tokenizer_path: Tokenizer 路径
            db_manager: 数据库管理器，用于查询历史请求进行前缀匹配
        """
        self.model = model
    ...
    async def encode_text(
        ...
        # 如果没有 session_id 或 db_manager，直接编码
        if not context.session_id or not self.db_manager:
            token_ids = self.tokenizer.encode(text, add_special_tokens=False)
            context.uncached_token_ids = token_ids
            return token_ids

        # 查询该 session 的所有历史请求
        history = await self.db_manager.get_request_records_by_session(context.session_id)
        ...
        if matched_trajectory:
            # 使用缓存的完整对话 token_ids
            cached_tokens = matched_trajectory["full_conversation_token_ids"]
            cached_text = matched_trajectory["full_conversation_text"]

            # 缓存命中的 token 数量
            context.cache_hit_tokens = len(cached_tokens)
            context.cached_token_ids = cached_tokens
