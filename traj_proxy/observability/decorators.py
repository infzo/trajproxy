...
def classify_infer_error(exc: Exception) -> str:
    ...
    通过回溯 exc.__cause__（原始 requests 异常）精确分类，
    不依赖错误消息字符串匹配。
    ...
    if isinstance(exc, InferTimeoutError):
        if isinstance(cause, (requests.exceptions.ConnectTimeout, requests.exceptions.ConnectionError)):
            return "connect_timeout"
        return "read_timeout"

    if isinstance(exc, InferServiceError):
        if isinstance(cause, requests.exceptions.HTTPError):
            if cause.response is not None and cause.response.status_code == 429:
                return "rate_limited"
            return "http_error"
        # ConnectionError / Timeout 等网络层错误
