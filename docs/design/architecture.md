...
> **导航**: [文档中心](../README.md) | [API 概览](../api/overview.md) | [开发指南](../guide/development.md)
...
| **TokenizerCache** | 按 tokenizer_path 共享 tokenizer 实例（引用计数） | `proxy_core/tokenizer_cache.py` |
...
┌─────────────────────────────────────────┐
│         InferClient                      │
│  注入 logprobs=1, return_token_ids=True │
...
┌─────────────────────────────────────────┐
│     响应处理                             │
│  存储 raw_response（含 logprobs/         │
│  token_ids）到数据库                      │
│  从客户端响应中剥离 logprobs/token_ids   │
└──────────┬──────────────────────────────┘
           │
           ▼
OpenAI Response（不含 logprobs/token_ids）
```
...
**职责：**
- 转发请求到推理服务（强制注入 logprobs=1, return_token_ids=True）
- 从客户端响应中剥离 logprobs/token_ids（仅保留在 raw_response 用于 DB 存储）
- 处理流式响应的字段累积
- 构建 OpenAI 格式响应

**核心特性：**
- 向推理服务强制注入 `logprobs=1` 和 `return_token_ids=True` 用于轨迹记录
- 从客户端响应中剥离 `logprobs` 和 `token_ids` 字段，仅保留在 `raw_response` 中用于 DB 存储
- 支持流式 tool_calls 合并
...
```python
class Processor:
    def __init__(self, model, tokenizer_path, config, tokenizer=None, ...):
        self.token_in_token_out = config.get("token_in_token_out", False)
        self._tokenizer = tokenizer  # 由 ProcessorManager 通过 TokenizerCache 注入
...
**特性：**
- 使用 httpx.AsyncClient 实现纯异步 HTTP 调用，无需线程池
- 内置重试机制（502/503/504 自动重试，指数退避）
- 连接池管理（httpx.Limits，可配置最大连接数）
- 支持 Chat Completions → Completions 参数自动映射与不兼容参数丢弃
...
```python
# 轻量配置存储（注册时仅存 ModelConfig，不创建 Processor）
_config_configs: Dict[Tuple[run_id, model_name], ModelConfig]   # 预置模型（来自 config YAML）
_dynamic_configs: Dict[Tuple[run_id, model_name], ModelConfig]  # 动态模型（来自 DB 同步）

# LRU 缓存：首次请求时才创建 Processor，缓存满时自动淘汰最久未使用
_processor_cache: OrderedDict[Tuple[run_id, model_name], Processor]

# 缓存配置项
processor_cache_max_size: int   # LRU 缓存上限（默认由配置决定）
processor_idle_timeout: int     # 空闲淘汰超时秒数（0 表示禁用）
```

**核心方法：**

```python
# 注册动态模型（仅存配置 + 持久化到数据库，Processor 懒加载）
async def register_dynamic_processor(model_name, url, api_key, ...) -> ModelConfig

# 注册预置模型（仅内存，Processor 懒加载）
def register_static_processor(model_name, url, api_key, ...)

# 删除动态模型（从配置和缓存中移除 + 从数据库删除）
async def unregister_dynamic_processor(model_name, persist_to_db=True, run_id="") -> bool

# 获取 Processor（LRU 缓存命中或懒加载，带锁保护）
async def get_processor_async(run_id, model_name) -> Optional[Processor]

# 从数据库查询并同步模型（缓存未命中时的回退机制）
async def try_get_or_sync_from_db(run_id, model_name) -> Optional[Processor]

# ModelSynchronizer 回调接口
async def register_from_config(config: ModelConfig)        # 增量注册
async def unregister_by_key(key: Tuple)                     # 增量删除
async def full_sync(db_models: List[ModelConfig])           # 全量同步

# 查询接口
def list_models() -> List[Tuple[str, str]]
def get_processor_info(run_id, model_name) -> Optional[Dict]
def get_all_processors_info() -> List[Dict]

# 空闲淘汰管理
async def start_idle_eviction()    # 启动空闲淘汰后台任务
async def stop_idle_eviction()     # 停止空闲淘汰后台任务
async def clear_cache()            # 清理所有缓存中的 Processor
```
...
> **详细设计**：参见 [ID 设计规范](modules/identifiers.md)
...
class ProcessContext:
    ...
    # 分阶段耗时（毫秒）
    transform_duration_ms: Optional[float] = None    # 消息转换耗时（TITO 模式）
    encode_duration_ms: Optional[float] = None       # Token编码总耗时（TITO 模式）
    cache_db_query_ms: Optional[float] = None        # 缓存DB查询耗时（TITO 模式）
    cache_prefix_match_ms: Optional[float] = None    # 前缀匹配耗时（TITO 模式）
    inference_duration_ms: Optional[float] = None    # 推理请求耗时
    decode_duration_ms: Optional[float] = None       # Token解码耗时（TITO 模式）
    ...
    cache_hit_tokens: Optional[int] = None      # 缓存命中的 token 数量（仅 TITO 模式填充）
    ...
    stream_logprobs: Optional[Dict] = None            # logprobs（仅用于 DB 存储，不返回客户端）
    stream_stop_reason: Optional[Any] = None          # vLLM 扩展
    stream_token_ids: Optional[List[int]] = None      # vLLM 扩展（仅用于 DB 存储）
    stream_prompt_token_ids: Optional[List[int]] = None  # vLLM 扩展：prompt 的 token ID 列表（首 chunk）
    stream_response_metadata: Optional[Dict] = None   # 累积的顶级元数据（id, model, created 等）
    stream_usage_full: Optional[Dict] = None          # 完整的 usage 对象（保留 prompt_tokens_details 等）
```
...
| 阶段 | Token-in-Token-out 模式 | 直接转发模式 |
|------|------------------------|--------------|
| 接收 | token_ids 或 text | OpenAI Response（含 logprobs/token_ids） |
| 转换 | token_ids → text → OpenAI Response | 剥离 logprobs/token_ids 后返回客户端 |
...
| cache_hit_tokens | 填充 | None（仅 TITO 模式由 PrefixMatchCache 填充） | metadata |
...
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 注册模型 | POST | `/models/register` | 注册新模型，自动同步到所有 Worker |
| 删除模型 | DELETE | `/models?model_name=&run_id=` | 删除动态模型，预置模型不可删除 |
| 列出模型（管理格式） | GET | `/models` | 返回所有模型的详细信息（含加载状态） |

### 10.2 聊天补全 API

| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 聊天补全（标准） | POST | `/v1/chat/completions` | 标准 OpenAI 聊天补全接口 |
| 聊天补全（带 session_id） | POST | `/s/{session_id}/v1/chat/completions` | 路径注入 session_id |
| 聊天补全（带 run_id + session_id） | POST | `/s/{run_id}/{session_id}/v1/chat/completions` | 路径注入 run_id 和 session_id |

### 10.3 模型列表 API（OpenAI 兼容）

| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 列出模型（OpenAI 格式） | GET | `/v1/models` | 返回 `{"object": "list", "data": [...]}` 格式 |

### 10.4 轨迹查询 API

| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 查询轨迹（旧接口） | GET | `/trajectory?session_id=&limit=&fields=` | 旧版轨迹查询（向后兼容） |
| 轨迹列表 | GET | `/trajectories?run_id=` | 按 run_id 查询轨迹列表 |
| 轨迹详情 | GET | `/trajectories/{session_id}?limit=&fields=` | 按 session_id 查询轨迹详情 |
...
聊天补全请求支持通过 Header 注入 `x-run-id`、`x-session-id`、`x-sandbox-traj-id`。

> **完整提取优先级规则**参见 [ID 设计规范](modules/identifiers.md)。
...
| Header | 说明 |
|--------|------|
| `x-session-id` | 会话 ID（用于轨迹分组） |
| `x-sandbox-traj-id` | 沙箱追踪 ID（回退为 session_id） |
| `x-run-id` | 运行 ID（用于模型路由） |

非黑名单 Header 会自动转发到推理服务（黑名单：host, content-length, content-type, connection, accept, accept-encoding, user-agent, authorization, x-run-id, x-session-id）。
...
| Tokenizer 缓存 | `traj_proxy/proxy_core/tokenizer_cache.py` |
...
WorkerManager 通过 Ray Actor 的 `max_restarts` 和 `max_task_retries` 配置提供多层容错：

| 配置项 | 值 | 行为 |
|--------|-----|------|
| `max_restarts` | 3 | Actor OOM / panic 后 Ray 自动重启 Actor（`__init__` 重执行，`initialize()` 不会自动重调） |
| `max_task_retries` | 2 | Worker 方法调用失败后最多重试 2 次 |

**自动恢复流程**：

1. Ray 检测到 Actor 崩溃后，自动重启 Actor（最多 3 次）
2. `_health_monitor_loop` 每 30 秒检测 Actor 存活状态
3. 发现 `_initialized=False` 的 Actor（重启后未重新初始化），调用 `_reinitialize_worker` 恢复 uvicorn 服务和业务组件
4. 恢复完成，Worker 继续正常处理请求

### 15.2 永久死亡场景与限制

当 `max_restarts`（3 次）耗尽后，Actor handle 失效，Worker 进入永久死亡状态：

- Ray 不会再自动重启该 Actor
- WorkerManager 无法通过任何机制恢复该 Worker（已放弃命名 Actor，避免非优雅重启后的名称冲突）
- `_recover_dead_worker` 仅记录错误日志，等待外部监控重建整个 WorkerManager 进程

**触发条件**（所有场景均需要外部干预）：

- Actor 在短时间内连续崩溃超过 3 次（如反复 OOM、死循环）
- Ray 集群级故障导致 Actor 不可恢复
- WorkerManager 进程自身崩溃（OOM / kill -9），所有 Worker handle 同时失效

### 15.3 部署要求（强制）

**生产环境必须配置外部监控机制，在 Worker 永久死亡或 WorkerManager 进程崩溃时自动重建服务。**

| 部署方式 | 推荐监控机制 | 配置要点 |
|----------|-------------|----------|
| Kubernetes | Pod liveness probe + restartPolicy | 探测 `/health` 端点，失败时自动重启 Pod |
| Docker Compose | `restart: unless-stopped` | 容器退出时 Docker 自动重启 |
| All-in-One (supervisord) | `autorestart=true` | supervisord 自动重启崩溃进程 |
| systemd | `Restart=always` + `RestartSec=5` | systemd 自动重启服务 |
| 超级进程管理 | `restartsecs=5` + `autorestart=true` | 进程崩溃后自动拉起 |
...
> **重要**：仅靠 Ray 的 `max_restarts` 不足以保证生产可用性。`max_restarts` 耗尽后的永久死亡场景必须依赖外部监控恢复，这是 TrajProxy 的部署契约。
